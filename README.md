...
4. **Point the code at your MySQL login.** Copy `.env.example` to a
   new file named `.env` in the project root, then fill in your own
   MySQL password:
   ```bash
   cp .env.example .env
   # then edit .env and set DB_PASSWORD to your MySQL root password
   ```
   `db.py` loads `.env` automatically (via `python-dotenv`) every
   time any script in this project runs, so this only has to be done
   once per machine — you don't need to `export` anything by hand or
   re-set it every terminal session. (You can still use `export`
   instead if you prefer; env vars set that way take priority over
   `.env`.) `.env` is your personal, machine-specific file and should
   **not** be committed — only `.env.example` (the blank template) is
   checked in, so this repo is safe to publish publicly (e.g. on
   GitHub) as-is.

   > **"Access denied ... (using password: NO)"** — this means
   > `DB_PASSWORD` isn't actually reaching the code (usually because
   > `.env` doesn't exist yet, or was never filled in) so it's
   > falling back to an empty password. Create/edit `.env` as above.
   > If you don't remember your MySQL root password, you can reset it
   > (see MySQL's docs for your OS) or create a dedicated app user:
   > ```sql
   > CREATE USER 'appuser'@'localhost' IDENTIFIED BY 'somepassword';
   > GRANT ALL PRIVILEGES ON automobile_db.* TO 'appuser'@'localhost';
   > FLUSH PRIVILEGES;
   > ```
   > then set `DB_USER=appuser` and `DB_PASSWORD=somepassword` in `.env`.
